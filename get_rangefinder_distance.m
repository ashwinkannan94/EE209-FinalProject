...
%     angle_reference = mod(theta + 90, 360);
    sensor_angle = mod(theta, 360);
%     if angle_reference > 0 && angle_reference < 180 % if the sensor is facing to the right
...
%     elseif angle_reference > 180 && angle_reference < 360 % if the sensor is facing to the left
%         slope = tan(theta*pi/180);
%         y_intercept = (pos_y - slope*pos_x);
%         min_x = max((env_size_y-y_intercept)/slope, 1); % if the slope is too steep, the max x is limited
%         x = linspace(min_x, pos_x, pos_x);
%         y = slope*x + y_intercept;
%     elseif angle_reference == 180 % if the sensor is facing straight up
%         y = (pos_y: env_size_y);
%         x = pos_x*ones(length(y),1)';
%     elseif angle_reference == 0 % if the sensor is facing straight down
%         y = (1: pos_y);
%         x = pos_x*ones(length(y),1)';
%     end
